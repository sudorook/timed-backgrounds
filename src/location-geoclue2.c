// SPDX-FileCopyrightText: 2014 - 2017 Jon Lund Steffensen <jonlst@gmail.com>
                         ...
                         gpointer user_data)
{
  ...
  GError* error = NULL;
  GDBusProxy* location =
    g_dbus_proxy_new_sync(g_dbus_proxy_get_connection(client),
  ...
  GVariant* lat_v = g_dbus_proxy_get_cached_property(location, "Latitude");
  state->latitude = g_variant_get_double(lat_v);

  GVariant* lon_v = g_dbus_proxy_get_cached_property(location, "Longitude");
...
static void
on_name_appeared(GDBusConnection* conn,
                 const gchar* name,
                 ...
                 gpointer user_data)
{
  ...
  error = NULL;
  GVariant* client_path_v = g_dbus_proxy_call_sync(geoclue_manager,
                                                   "GetClient",
                                                   NULL,
  ...
  g_signal_connect(geoclue_client,
                   "g-signal",
  ...
  ret_v = g_dbus_proxy_call_sync(
    geoclue_client, "Start", NULL, G_DBUS_CALL_FLAGS_NONE, -1, NULL, &error);
  if (ret_v == NULL) {
    g_printerr(_("Unable to start GeoClue client: %s.\n"), error->message);
    if (g_dbus_error_is_remote_error(error)) {
      gchar* dbus_error = g_dbus_error_get_remote_error(error);
...
run_geoclue2_loop(void* state_)
{
  ...
  GSource* pipe_source =
    g_io_create_watch(pipe_channel, G_IO_IN | G_IO_HUP | G_IO_ERR);
...
static int
location_geoclue2_handle(location_geoclue2_state_t* state,
                         location_t* location,
                         int* available)
{
